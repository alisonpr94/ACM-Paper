...
def save_files(ID, HS, TR, AG):
    with open("ClassPred/RF_BoW.tsv", "w") as file:
...
def classify(x_train, y_train, x_test):
    ...
    clf = RandomForestClassifier(n_estimators=100, max_features='log2', class_weight='balanced_subsample',random_state=None)

    clf.fit(x_train, y_train)   # Fase de treinamento

    # Criando arquivo para salvar modelo treinado
    filename = 'Models/modelRF.sav'
