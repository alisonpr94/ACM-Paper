...
def bag_of_words(train, test):
    vec = CountVectorizer(analyzer='word', binary=True, ngram_range=(1, 3), min_df=1, max_features=50000)
...
def save_files(y_test, y_pred, ID):
    ...
    with open("input/res/es_a.tsv", "w") as file:
        for i in range(len(y_pred)):
            file.write(str(ID[i]))
            file.write('\t')
            file.write(str(y_pred[i]))
...
def main():
	...
	rf = RandomForestClassifier(n_estimators=100, max_features='log2', class_weight='balanced_subsample',random_state=42)
	lr = LogisticRegression(C=10, tol=1e-3, random_state=None, solver='liblinear', verbose=1, max_iter=1000)
	svc = LinearSVC(C=10, verbose=1, max_iter=2000, random_state=None)
