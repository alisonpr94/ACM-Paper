...
def save_files(y_test, y_pred, ID):
	with open("ClassPred/LR_BoW.tsv", "w") as file:
...
def main():
	...
	clf = LogisticRegression(C=10, tol=1e-3, random_state=None, solver='liblinear', verbose=1, max_iter=1000)

	clf.fit(x_train, y_train)   # Fase de treinamento

	# Criando arquivo para salvar modelo treinado
	filename = 'Models/modelLogisticRegression.sav'
